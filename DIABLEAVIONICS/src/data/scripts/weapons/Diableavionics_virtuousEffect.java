...
public class Diableavionics_virtuousEffect implements EveryFrameWeaponEffectPlugin {
...
//    private final String ID = "DiableAvionics_virtuous";
    private final String section = "diableavionics";
    
    //FLICKER VALUES
    private final String FLICKER_ID = "diableavionics_unlockedFlicker";
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        float lAngle=ship.getFacing();
        lAngle+=MathUtils.getShortestRotation(lAngle, larm.getCurrAngle())/2;
        lpauldron.setCurrAngle(lAngle);
        lpauldronglow.setCurrAngle(lAngle);
        
        float rAngle=ship.getFacing();
        rAngle+=MathUtils.getShortestRotation(rAngle, rarm.getCurrAngle())/2;
        rpauldron.setCurrAngle(rAngle);
        rpauldronglow.setCurrAngle(rAngle);
    ...
    private void flickerEffect(){
        ...
        lpauldronglow.getSprite().setColor(new Color(1,level,0.5f+level/2,level));
        rpauldronglow.getSprite().setColor(new Color(1,level,0.5f+level/2,level));

//            //messing up stats
//            ship.getMutableStats().getAcceleration().modifyPercent(ID, 2000*level);
//            ship.getMutableStats().getDeceleration().modifyPercent(ID, 2000*level);
//            ship.getMutableStats().getTurnAcceleration().modifyPercent(ID, 2000*level);
//            ship.getMutableStats().getMaxSpeed().modifyPercent(ID, 100*level);
//            ship.getMutableStats().getMaxSpeed().modifyMult(ID, 1-(level/2));
