...
public class Diableavionics_deepStrikeAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    private IntervalUtil jitter = new IntervalUtil(0.08f,0.12f);
...
//        ShipAPI teamate = engine.getFleetManager(missile.getOwner()).spawnShipOrWing(WANZERS.get(MathUtils.getRandomNumberInRange(0, WANZERS.size()-1)), missile.getLocation(), missile.getFacing());
        engine.getFleetManager(2).setSuppressDeploymentMessages(true);
        engine.getFleetManager(missile.getOwner()).setSuppressDeploymentMessages(true);
        ShipAPI dropPod = engine.getFleetManager(2).spawnShipOrWing("diableavionics_deepStrike_"+MathUtils.getRandomNumberInRange(1, 5), missile.getLocation(), missile.getFacing());
        engine.getFleetManager(2).setSuppressDeploymentMessages(false);
