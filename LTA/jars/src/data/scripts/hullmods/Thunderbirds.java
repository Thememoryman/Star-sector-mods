...
public class Thunderbirds extends BaseHullMod {
	...
	private static final float RECOILMAX_MULT = 0.85f;
        ...
        private static final float MISSILEAMMOMAX_INCREASEPERCENT = 25f;
	...
	public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
		...
		//stats.getEngineMalfunctionChance().modifyFlat(id, ENGINE_MALFUNCTION_PROB);
                ...
                stats.getMissileAmmoBonus().modifyPercent(id, MISSILEAMMOMAX_INCREASEPERCENT);
	...
	public String getDescriptionParam(int index, HullSize hullSize, ShipAPI ship) {
		...
		if (index == 0) return "" + (int) Math.round((1f - HULL_MULT) * 100f) + "%";
        	...
        	if (index == 2) return "" + (int) Math.round((1f - WEAPON_DAMAGE_TAKEN_MULT) * 100f) + "%";
                ...
                if (index == 5) return "" + (int) MISSILEAMMOMAX_INCREASEPERCENT + "%";
		if (index == 6) return "" + (int) Math.round((SUPPLIES_TO_RECOVER - 1f) * 100f) + "%";
		if (index == 7) return "" + (int) Math.round((SUPPLIES_PER_MONTH - 1f) * 100f) + "%";
		return null;
	}
}
