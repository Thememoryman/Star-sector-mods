...
public class LTA_NyxDrone extends BaseEveryFrameCombatPlugin {
    
    private CombatEngineAPI engine;
    
    private final IntervalUtil DARKSUMMONINGRITUALFROMBEYONDTHEGREYSUN = new IntervalUtil(0.1f, 0.1f);   
    
    private final IntervalUtil interval = new IntervalUtil(0.1f, 0.1f);
    private float timer = 0f;       //Timer
    private float duration = 0.05f; //Duration of the effect
    
    private static final String LTA_NyxDroneShipID = "LTA_NyxDrone_Wing";
    private static final String LTA_NyxDroneProjectileID = "LTA_nyxdrone_mrm2";
    ...
    public void advance(float amount, List<InputEventAPI> events) {
        ...
        while (iter.hasNext()) {
            ...
            if (DARKSUMMONINGRITUALFROMBEYONDTHEGREYSUN.getElapsed() >= 0.1f) {
            switch (projectile.getProjectileSpecId()) {
                case LTA_NyxDroneProjectileID: {
                    ...
                    FleetMemberAPI missileMember = Global.getFactory().createFleetMember(FleetMemberType.FIGHTER_WING, LTA_NyxDroneShipID);
            ...
            DARKSUMMONINGRITUALFROMBEYONDTHEGREYSUN.advance(amount);
        }
    }
