...
public class EpattcudXVIIITeleporterStats extends BaseShipSystemScript {
			...
			ship.getEngineController().extendFlame(this, 2f * effectLevel, 0f * effectLevel, 0f * effectLevel);
                            ...
                            for (int x = 0; x < 60; x++) {
                                Global.getCombatEngine().spawnEmpArc(ship, ship.getLocation(),
                                                   empTarget,
                                                   empTarget, DamageType.ENERGY, 65, 100,
                                                   2000, null, 30f, new Color(230,40,40,180),
                                                   new Color(255,255,255,255));
                            }	

                            for (CombatEntityAPI EmpTarget :AIUtils.getNearbyEnemies(ship,800)) {
                                for (int x = 0; x < 15; x++) {
                                
                                Global.getCombatEngine().spawnEmpArc(ship, ship.getLocation(),
                                                   EmpTarget,
                                                   EmpTarget, DamageType.ENERGY, 300, 450,
                            ...
                            for (CombatEntityAPI EmpTarget :AIUtils.getNearbyAllies(ship,800)) {
                                for (int x = 0; x < 15; x++) {
                                
                                Global.getCombatEngine().spawnEmpArc(ship, ship.getLocation(),
                                                   EmpTarget,
                                                   EmpTarget, DamageType.ENERGY, 25, 450,
                                                   2000, null, 30f, new Color(230,40,40,255),
                                                   new Color(255,255,255,255));
                                }
                            }
