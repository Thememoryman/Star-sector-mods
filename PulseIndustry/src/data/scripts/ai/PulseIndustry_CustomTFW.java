...
public class PulseIndustry_CustomTFW implements MissileAIPlugin, GuidedMissileAI {
    ...
    public PulseIndustry_CustomTFW(MissileAPI missile, ShipAPI launchingShip) {
    ...
    public void advance(float amount) {
        if (missile.isFizzling() || !missile.isArmed() || missile.isFading() || ((missile.getMaxHitpoints() / 2) > missile.getHitpoints())) {
            ...
            Global.getCombatEngine().addSmoothParticle(missile.getLocation(), zero, 90, 0.5f, 1, Color.PINK);
        ...
        if(acc<=0)missile.giveCommand(ShipCommand.ACCELERATE);
